...
read_cf_dns_api_token() {
  echo "" > /dev/stderr
  echo "Cloudflare DNS-01 challenge is used for Let's Encrypt (required for non-standard ports)." > /dev/stderr
  echo "You can also set the CF_DNS_API_TOKEN environment variable before running this script." > /dev/stderr
  echo -n "Cloudflare API Token (DNS:Edit permission): " > /dev/stderr
  read -r TOKEN < /dev/tty
  if [[ -z "$TOKEN" ]]; then
    echo "Cloudflare API Token is required for DNS-01 challenge." > /dev/stderr
    read_cf_dns_api_token
    return
  fi
  echo "$TOKEN"
...
configure_reverse_proxy() {
  ...
  if [[ "$REVERSE_PROXY_TYPE" == "0" ]]; then
    ...
    if [[ -z "$CF_DNS_API_TOKEN" ]]; then
      CF_DNS_API_TOKEN=$(read_cf_dns_api_token)
    else
      echo "" > /dev/stderr
      echo "Using Cloudflare API Token from CF_DNS_API_TOKEN environment variable." > /dev/stderr
    fi
...
start_services_and_show_instructions() {
  ...
  if [[ "$REVERSE_PROXY_TYPE" == "0" ]]; then
    ...
    echo "  $NETBIRD_HTTP_PROTOCOL://$NETBIRD_DOMAIN:$NETBIRD_PORT"
...
print_builtin_traefik_instructions() {
  ...
  echo "using Cloudflare DNS-01 challenge (works with non-standard ports)."
  echo "If you see certificate warnings, wait a moment for DNS propagation and certificate issuance."
  echo ""
  echo "Open ports:"
  echo "  - 18443/tcp (HTTPS - all NetBird services)"
  echo "  - 18080/tcp (HTTP - redirects to HTTPS)"
